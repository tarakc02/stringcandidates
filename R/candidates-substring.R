...
substring_candidates2 <- function(a, b, comparison, k = 6,
                                 output = NULL) {
    comp <- parse_comparison(a, b, comparison)
    compa <- comp$compa
    compb <- comp$compb
...
substring_inner_join2 <- function(a, b, by, k) {
    compa <- a[, names(by), drop = FALSE]
    compb <- b[, by, drop = FALSE]
    matches <- lapply(
        seq_along(by),
        function(i) match_substrings2(compa[[i]], compb[[i]], k = k)
    )
